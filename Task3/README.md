...
| Apache Camel (ESB)                    | Выкидываем         | Платформы               | Выводится из эксплуатации. Его функции (маршрутизация, трансформации, оркестрация, обработка ошибок) распределяются между Kafka (передача событий), потоковой обработкой (Flink/Kafka Streams), пакетной обработкой (Spark/dbt) и оркестратором (Airflow). Это устраняет единую точку отказа и повышает масштабируемость интеграций. |
...
Действия:
1. Внедрить кластер Kafka (или управляемый Event Hubs) как центральную шину событий.
2. Внедрить компоненты потоковой обработки (Kafka Streams / Apache Flink) для выполнения маршрутизации, фильтрации и обогащения событий в реальном времени.
3. Переключить интеграционные потоки с ESB на событийно-ориентированную архитектуру: домены (Клиники, Финтех, ИИ) начинают публиковать события изменений в Kafka.
4. Настроить потоковую обработку для наполнения Data Lake (Silver-слой: очищенные, проверенные данные).
5. Внедрить оркестратор (Airflow) для управления пакетными заданиями, которые будут строиться на следующем этапе.
6. Настроить мониторинг задержек и пропускной способности, Dead Letter Queue для обработки ошибок.
...
Необходимые ресурсы:  
Инженеры Kafka, специалисты по потоковой обработке (Flink/Spark Streaming), инженеры по оркестрации.

Обоснование:  
ESB — узкое место для масштабирования. Переход на EDA с распределённой обработкой позволит подключить производителя электроники за недели, а не месяцы, и повысит отказоустойчивость (события буферизируются, а обработка децентрализована).
...
Действия:
1. На основе Silver-слоя построить Gold-витрины для каждого домена (Клиники, Финтех, ИИ) с использованием dbt (пакетная обработка).
...
5. Запуск пилотного проекта по интеграции с фармацевтическим партнёром через Kafka и потоковую обработку.
